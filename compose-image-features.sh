...
write_devcontainer() {
  ...
  for key in "${SELECTED_FEATURES[@]}"; do
    ...
    features_json+="\"$feature_ref\": $opts"
    first=0
  done
  features_json+="}"

  cat > .devcontainer/devcontainer.json <<'EOF_INNER'
{
  "image": "$FINAL_IMAGE",
...
EOF_INNER
  ...
  echo "✅ .devcontainer/devcontainer.json created at $(pwd)/.devcontainer/devcontainer.json"
}
